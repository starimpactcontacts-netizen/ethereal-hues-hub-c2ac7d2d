-- Update sync battle to use battle_songs pool (admin-curated, with cover art + difficulty)
-- Also adds scenepack thumbnail + song cover/difficulty to quick_fights
...
-- Replace spin_sync_fight to pick from battle_songs instead of sync_song_pool
...
BEGIN
  ...
  -- Random song from admin-uploaded battle_songs (excludes Deezer-sourced)
  SELECT * INTO v_song FROM battle_songs WHERE deezer_id IS NULL ORDER BY RANDOM() LIMIT 1;
  -- Fallback: include all if no manual uploads exist
  IF NOT FOUND THEN
