...
import { Plus, Trash2, Save, BarChart3, Eye, TrendingUp, Zap, MousePointerClick, Edit3, X, ChevronDown, ChevronUp } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useArtistCampaigns, useEnterpriseClients } from '@/hooks/useArtistCampaigns';
...
export default function CampaignAdminPage() {
  ...
  const [newCampaign, setNewCampaign] = useState({ client_id: '', name: '', description: '' });
  // Add edit form
  const [newEdit, setNewEdit] = useState({ title: '', video_url: '', thumbnail_url: '', platform: 'tiktok', editor_username: '', view_count: 0 });
  // Edit stats form
  const [statsForm, setStatsForm] = useState({ total_views: 0, total_impressions: 0, total_engagements: 0, total_clicks: 0, roi_percentage: 0 });
  ...
  const handleCreateCampaign = async () => {
    ...
    try {
      ...
      setNewCampaign({ client_id: '', name: '', description: '' });
