...
export default function MissionLobbyPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      {/* ═══ ENTER LOBBY — Big CTA to Official Event or Drop ═══ */}
      {(() => {
        const lobbyTarget = officialEvent || { id: mission.id, title: mission.title, prize_usd: maxPayout > 0 ? maxPayout : 0, poster_url: mission.poster_url };
        return (
          <motion.div 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.15em]">
                        {officialEvent ? 'Battle Royale' : 'Competition Lobby'}
                      </span>
                    </div>
                    <h3 style={teko} className="text-[17px] text-white leading-tight truncate tracking-wide">
                      {lobbyTarget.title}
                    </h3>
                    {lobbyTarget.prize_usd > 0 && (
                      <div className="flex items-center gap-2 mt-1">
                        <DollarSign className="w-3 h-3 text-emerald-400" />
                        <span className="text-[12px] font-black text-emerald-400">${lobbyTarget.prize_usd} Prize</span>
                      </div>
                    )}
                  </div>
