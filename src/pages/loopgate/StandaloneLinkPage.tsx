  ...
  return (
    <div className="min-h-screen" style={{ ...getBackground(settings), color: textColor }}>
      <div className="max-w-[440px] mx-auto px-5 pt-16 pb-10 min-h-screen flex flex-col">
        ...
        {/* Editor Stats — compact inline */}
        {showStats && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.12 }} className="mb-5">
            <div className="flex items-center justify-center gap-4">
              <div className="flex flex-col items-center">
                <p className="text-sm font-bold text-white/70">{classInfo.letter}</p>
                <span className="text-[8px] text-white/25 font-medium uppercase tracking-wider">Class</span>
              </div>
              <div className="w-px h-5 bg-white/8" />
              <div className="flex flex-col items-center">
                <p className="text-sm font-bold tabular-nums text-white/70">#{profile.global_rank || '—'}</p>
                <span className="text-[8px] text-white/25 font-medium uppercase tracking-wider">Rank</span>
              </div>
              <div className="w-px h-5 bg-white/8" />
              <div className="flex flex-col items-center">
                <div className="flex gap-px">
                  {[1,2,3,4,5].map(s => (
                    <svg key={s} className={`w-2.5 h-2.5 ${s <= starRating ? 'text-white/60' : 'text-white/10'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <span className="text-[8px] text-white/25 font-medium uppercase tracking-wider mt-px">Trust</span>
              </div>
            </div>
            <p className="text-[8px] text-white/15 text-center mt-1.5 tracking-wider uppercase">Global Editor Index</p>
          </motion.div>
        )}
