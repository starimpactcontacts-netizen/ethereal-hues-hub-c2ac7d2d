...
export default function ChannelSettingsPage({
  ...
  const resetCommand = () => {
    ...
    setEmbedTitle(""); setEmbedBody("");
    setPollTitle(""); setPollOptions(["", ""]); setPollDuration("24");
    setEventTitle(""); setEventDescription(""); setEventDate(""); setEventTime("");
    setReminderText("");
    setRules(["", ""]);
  };

  const handleEmbed = async () => { if (!embedBody.trim()) return; await postEmbed(embedTitle, embedBody); resetCommand(); };
  const handlePoll = async () => { const v = pollOptions.filter(o => o.trim()); if (!pollTitle.trim() || v.length < 2) return; await createPoll(pollTitle.trim(), v, parseInt(pollDuration)); resetCommand(); };
  const handleEvent = async () => { if (!eventTitle.trim() || !eventDate || !eventTime) return; await createEvent(eventTitle.trim(), eventDescription.trim(), `${eventDate}T${eventTime}`); resetCommand(); };
  const handleReminder = async () => { if (!reminderText.trim()) return; await createReminder(reminderText.trim()); resetCommand(); };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border/50 shrink-0">
        <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted/50 transition-colors -ml-1">
          <ArrowLeft className="w-5 h-5 text-muted-foreground" />
        </button>
        <div className="flex-1 min-w-0">
          <h1 className="text-sm font-semibold">Channel Settings</h1>
        ...
        <div className="p-4 space-y-5 max-w-lg mx-auto">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h2 className="text-sm font-bold uppercase tracking-wide">{botName || "Unit Bot"}</h2>
            ...
            {!activeCommand ? (
              <div className="space-y-1">
                {commands.map((cmd) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-3.5 rounded-xl hover:bg-muted/30 transition-colors text-left active:scale-[0.98] touch-manipulation"
                  >
              ...
              <div className="space-y-3 bg-muted/10 border border-border/30 rounded-xl p-4">
                ...
                {activeCommand === "embed" && (
                  <>
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold flex items-center gap-2"><Send className="w-4 h-4 text-primary" /> Embed Message</h3>
                ...
                {activeCommand === "poll" && (
                  <>
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold flex items-center gap-2"><BarChart3 className="w-4 h-4 text-primary" /> Create Poll</h3>
                      <button onClick={resetCommand} className="p-1 rounded hover:bg-muted/50"><X className="w-4 h-4 text-muted-foreground" /></button>
                    </div>
                    ...
                    <div className="space-y-2">
                      {pollOptions.map((opt, i) => (
                        <div key={i} className="flex items-center gap-2">
                          ...
                          {pollOptions.length > 2 && <button onClick={() => setPollOptions(pollOptions.filter((_, j) => j !== i))} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></button>}
                        </div>
                      ))}
                      {pollOptions.length < 6 && <button onClick={() => setPollOptions([...pollOptions, ""])} className="text-xs text-primary flex items-center gap-1 px-1"><Plus className="w-3 h-3" /> Add option</button>}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <select value={pollDuration} onChange={(e) => setPollDuration(e.target.value)} className="bg-muted/30 rounded px-2 py-1 text-xs border border-border">
                        <option value="1">1 hour</option><option value="6">6 hours</option><option value="24">24 hours</option><option value="72">3 days</option><option value="168">7 days</option>
                      </select>
                    </div>
                    <Button onClick={handlePoll} disabled={sending || !pollTitle.trim() || pollOptions.filter(o => o.trim()).length < 2} className="w-full" size="sm">{sending ? "Creating..." : "Create Poll"}</Button>
                  </>
                )}
                ...
                {activeCommand === "event" && (
                  <>
                    ...
                    <Button onClick={handleEvent} disabled={sending || !eventTitle.trim() || !eventDate || !eventTime} className="w-full" size="sm">{sending ? "Scheduling..." : "Schedule Event"}</Button>
                  </>
                )}
                ...
                {activeCommand === "reminder" && (
                  <>
                    ...
                    <Textarea placeholder="What do you want to remind the unit about?" value={reminderText} onChange={(e) => setReminderText(e.target.value)} className="text-sm min-h-[100px]" />
                    <Button onClick={handleReminder} disabled={sending || !reminderText.trim()} className="w-full" size="sm">{sending ? "Posting..." : "Post Reminder"}</Button>
                  </>
                )}
                    ...
                    <div className="space-y-2">
                      {rules.map((rule, i) => (
                        <div key={i} className="flex items-center gap-2">
                          ...
                          {rules.length > 1 && <button onClick={() => setRules(rules.filter((_, j) => j !== i))} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></button>}
                        </div>
                      ))}
                      {rules.length < 10 && <button onClick={() => setRules([...rules, ""])} className="text-xs text-primary flex items-center gap-1 px-1"><Plus className="w-3 h-3" /> Add rule</button>}
                    </div>
                    <Button onClick={handleRules} disabled={sending || rules.filter(r => r.trim()).length === 0} className="w-full" size="sm">{sending ? "Posting..." : "Post Rules"}</Button>
                  </>
                )}
              </div>
            )}
          ...
          {onSaveBotSettings && (
            <div className="border-t border-border/30 pt-4">
              <button onClick={() => setShowBotIdentity(!showBotIdentity)} className="w-full flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-primary" />
                  <span className="text-sm font-semibold">Bot Identity</span>
                </div>
          ...
          {/* ═══ CHANNEL PERMISSIONS ═══ */}
          <div className="border-t border-border/30 pt-4">
            <button onClick={() => setShowPermissions(!showPermissions)} className="w-full flex items-center justify-between py-2">
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4 text-muted-foreground" />
                <span className="text-sm font-semibold">Channel Permissions</span>
              </div>
          ...
          {/* Save button for identity/permissions */}
