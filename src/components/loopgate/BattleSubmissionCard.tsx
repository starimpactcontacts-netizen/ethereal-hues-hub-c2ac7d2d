...
export default function BattleSubmissionCard({
  ...
  const isDirectUpload = platform === "unknown";
  const innerCls = `block relative ${aspectClass} bg-surface-2 overflow-hidden group`;
    ...
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 px-2 py-1.5 flex items-center justify-between gap-1">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {!isDirectUpload && <ExternalLink className="w-3 h-3 text-white/70" />}
        </div>
      </div>
    </>
  );
  ...
  return (
    <div className={`bg-surface-1 border ${borderColor} ${hoverBorder} transition-all overflow-hidden ${isWinner ? "ring-2 ring-gold/50" : ""}`}>
      {isDirectUpload ? (
        <div className={innerCls}>{cardInner}</div>
      ) : (
        <a href={playbackUrl} target="_blank" rel="noopener noreferrer" className={innerCls}>{cardInner}</a>
      )}
    </div>
  );
}
