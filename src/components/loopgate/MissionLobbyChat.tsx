...
export default function MissionLobbyChat({ missionId }: MissionLobbyChatProps) {
  ...
  const handleReply = (msg: Message) => {
    setReplyTo({ id: msg.id, username: msg.username, text: msg.message_text.slice(0, 50) });
  ...
  const renderMessageText = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@')) {
        ...
        return (
          <button key={i} onClick={() => navigate(`/u/${username}`)} className="text-emerald-400 font-semibold hover:underline">
            {part}
          </button>
        );
      }
    ...
    <div className="relative overflow-hidden flex flex-col rounded-2xl bg-card/60 backdrop-blur-xl border border-border/50 shadow-lg">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-border/30">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground tracking-tight">Mission Chat</h3>
      ...
      {/* Messages */}
      <div ref={scrollContainerRef} onScroll={handleScroll} className="h-[300px] sm:h-72 overflow-y-auto px-3 py-3 space-y-3 relative">
        {loading ? (
              ...
              if (msg.is_system) {
                return (
                  <motion.div key={msg.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center py-1">
                    <span className="text-[10px] text-muted-foreground/50 bg-muted/30 px-3 py-1 rounded-full">{msg.message_text}</span>
                  </motion.div>
                );
              }

              return (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} className="group">
                  <div className={`flex gap-2.5 ${isOwn ? 'flex-row-reverse' : ''}`}>
                    {/* Avatar */}
                    <button onClick={() => navigate(`/u/${msg.username}`)} className="w-8 h-8 rounded-full overflow-hidden shrink-0 bg-muted/50 hover:ring-2 hover:ring-emerald-500/30 transition-all">
                      {msg.avatar_url ? (
                    ...
                    <div className={`max-w-[75%] ${isOwn ? 'items-end' : 'items-start'} flex flex-col`}>
                      ...
                      <div className={`flex items-center gap-2 mb-1 ${isOwn ? 'flex-row-reverse' : ''}`}>
                        <button onClick={() => handleMention(msg.username)} className="text-[11px] font-bold text-foreground hover:text-emerald-400 transition-colors">
                          {msg.username}
                        </button>
                      ...
                      {msg.reply_to_username && (
                        <div className={`flex items-start gap-1.5 mb-1.5 px-3 py-1.5 rounded-xl bg-muted/30 border border-border/30 max-w-full ${isOwn ? 'self-end' : 'self-start'}`}>
                          ...
                          <div className="min-w-0">
                            <span className="text-[10px] font-bold text-emerald-400/70">@{msg.reply_to_username}</span>
                            {msg.reply_to_text && <p className="text-[10px] text-muted-foreground/50 truncate">{msg.reply_to_text}</p>}
                          </div>
                        </div>
                      )}
                      ...
                      {/* Reply action */}
                      {user && (
                        <button onClick={() => handleReply(msg)} className={`opacity-0 group-hover:opacity-100 active:opacity-100 flex items-center gap-1 mt-0.5 px-2 py-0.5 text-[9px] text-muted-foreground/40 hover:text-emerald-400 transition-all touch-manipulation ${isOwn ? 'self-end' : 'self-start'}`}>
        ...
        {!isAtBottom && newCount > 0 && (
          <button onClick={scrollToBottom} className="sticky bottom-2 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-[11px] font-semibold px-4 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg shadow-emerald-500/20 z-10">
      ...
      <AnimatePresence>
        {showGifPicker && (
          <motion.div initial={{ height: 0 }} animate={{ height: 280 }} exit={{ height: 0 }} className="overflow-hidden border-t border-border/30">
      ...
      {user ? (
        <div className="border-t border-border/30">
          ...
          {replyTo && (
            <div className="flex items-center gap-2 px-4 py-2 bg-muted/20 border-b border-border/20">
              <Reply className="w-3.5 h-3.5 text-emerald-400 shrink-0 rotate-180" />
              <div className="flex-1 min-w-0">
                <span className="text-[11px] font-bold text-emerald-400">@{replyTo.username}</span>
          ...
          <div className="px-3 pt-2.5 pb-1 flex gap-1.5 overflow-x-auto scrollbar-hide">
            {QUICK_CHIPS.map((chip) => (
              <button key={chip} onClick={() => sendMessage(chip)} disabled={isSending}
                className="shrink-0 px-3 py-1 bg-muted/40 hover:bg-emerald-500/15 rounded-full text-[12px] font-medium text-muted-foreground hover:text-emerald-400 transition-all active:scale-95 touch-manipulation">
                {chip}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSend} className="px-3 pb-3 pt-1.5 flex gap-2 items-center">
            <button type="button" onClick={() => setShowGifPicker(!showGifPicker)}
              className={`w-9 h-9 shrink-0 flex items-center justify-center rounded-full transition-all touch-manipulation ${
                showGifPicker ? "bg-emerald-500/20 text-emerald-400" : "bg-muted/40 text-muted-foreground hover:text-foreground"
              }`}>
              <ImageIcon className="w-4 h-4" />
            </button>
            <input ref={inputRef} type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
              placeholder={replyTo ? `Reply to @${replyTo.username}...` : "Say something..."}
              className="flex-1 bg-muted/30 rounded-full px-4 py-2 text-[13px] text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 transition-all"
              maxLength={300} />
            <button type="submit" disabled={isSending || !newMessage.trim()}
              className="w-9 h-9 shrink-0 flex items-center justify-center rounded-full bg-emerald-500 hover:bg-emerald-400 disabled:opacity-30 disabled:hover:bg-emerald-500 text-white transition-all touch-manipulation active:scale-95">
        ...
        <div className="border-t border-border/30 p-4 text-center">
          <button onClick={() => navigate('/start')} className="text-xs font-semibold text-emerald-400 hover:text-emerald-300 transition-colors">
            Sign in to chat →
          </button>
        </div>
      )}
